 ...
 */
exports.authorize = function(requestData, requestType, callback) {
	if(client_secret){
		...
		fs.readFile(TOKEN_PATH, function(err, token) {
			if (err) {
				...
				if (requestType == "listFiles") {
					listFiles(requestData, oauth2Client, callback);
				} else if (requestType == "getFolderID") {
					getFolderID(requestData, oauth2Client, callback)
				}
			}
		});
	}else{
